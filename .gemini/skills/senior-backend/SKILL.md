...
How to design, validate, and implement new skills for gemini-code-java
Purpose
...
├── SKILL.md    # Instructions for Gemini (AI reads this)
...
Quality Standards
Content Guidelines
1. Be Specific to Java • Use Java syntax in examples • Reference Java ecosystem tools (Maven, JUnit, Spring) • Don't be generic - that's what other skills collections do
2. Provide Actionable Checklists • ✅ - [ ] Check for null safety with Optional • ❌ - [ ] Make sure code is good
3. Show Anti-Patterns // ❌ BAD: Why this is wrong badCode();  // ✅ GOOD: Why this is right goodCode();
4. Include Severity Levels • High: Bugs, security issues, performance killers • Medium: Code smells, maintainability issues • Low: Style, minor optimizations
Token Efficiency
Skills should help Gemini work efficiently:
...
Automated Review
PRs that modify .gemini/skills/ are automatically reviewed against these guidelines.
The review checks:
...
Gemini asks clarifying questions	Missing context or defaults	Add explicit defaults and examples
...
2. Note every correction you make to Gemini's output
